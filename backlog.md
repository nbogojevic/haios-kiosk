...
### Keep thumbnail generation off-main, but consider thumbnail cache.

CaptureThumbnailView.thumbnailImage(...) is already detached, which is good. A small in-memory cache keyed by image path + size would avoid regenerating thumbnails during navigation/list refreshes

### For future profiling, run a Release/Profile build without XCTest for final numbers.

The current trace was useful for finding code paths, but UI automation inflates accessibility/XCTest work. A manual Instruments run on a Release/Profile build would give cleaner absolute numbers
